...
//UsernamePasswordAuthenticationFilter는 스프링 시큐리티의 로그인 로직의 시작점
//UsernamePasswordAuthenticationFilter 클래스를 상속받아 새로운 클래스를 만든다는 것은
//UsernamePasswordAuthenticationFilter의 기능을 우리가 입맛대로 변경해서 사용하겠다는 의미.
@Slf4j
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
  private final AuthenticationManager authenticationManager;
  private final JwtUtil jwtUtil;

  //생성자 만들기 (우리 입맛에 맞게 변경하기 위해서)
  public LoginFilter(AuthenticationManager authenticationManager, JwtUtil jwtUtil){
    this.authenticationManager = authenticationManager;
    this.jwtUtil = jwtUtil;

    setFilterProcessesUrl("/users/login");  //로그인 요청 url 변경
    setUsernameParameter("memEmail");       //변경하지않으면 아이디는 username으로 전달
    setPasswordParameter("memPw");          //변경하지않으면 비번은 password로 전달
  }

  //attemptAuthentication() 메서드는 로그인 요청이 발생하는 가장 처음 호출됨
  //로그인 요청은 기본적으로 '/login'이다. (post) 바로 인지하고 실행해버린다.
  @Override
  public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
    ...
    //(58번줄) authToken : input 태그에 적은 아이디와 비밀번호를 담고 있는 객체
